...
class ProfileEmailUpdateView extends Component {
    ...
    componentDidMount() {
        document.title = "Change Email | Eiyoo"
    }
    ...
    updateEmail = e => {
        ...
        if (currentEmail != loginuserData.email) {
            this.setState({
                error: "Current email isn't correct",
                alertModal: true,
            })
            return
        }
        if (currentEmail == '') {
            this.setState({
                error: "Please input current email address",
                alertModal: true,
            })
            return
        }
        if (newEmail == '') {
            this.setState({
                error: "Please input new email address",
                alertModal: true,
            })
            return
        }
        if (newEmailConfirm == '') {
            this.setState({
                error: "Please confirm new email address",
                alertModal: true,
            })
            return
        }
        ...
        axios(config)
            ...
            .catch((error)=>{
                this.setState({
                    error: 'Another user uses that email',
                    alertModal: true
                })
            })
    }
            ...
            <>
                <div className="profile profile-email-input">
                    ...
                    <div className="profile-link">
                        <a onClick={this.updateEmail}>プロフィール設定</a>
